...
public class MainActivity extends AppCompatActivity {
    ...
    public void onStart() {
        ...
        if(currentUser==null){
            ...
            FirebaseDatabase.getInstance("https://chatting-a6254-default-rtdb.firebaseio.com/").getReference().child("users").child(CurrentUID).child("Seen").setValue("online");
        }
    ...
    protected void onStop() {
        ...
        if(isFinished) {
            ...
            if (currentUser != null) {
                String CurrentUID = currentUser.getUid();
                FirebaseDatabase.getInstance("https://chatting-a6254-default-rtdb.firebaseio.com/").getReference().child("users").child(CurrentUID).child("Online").setValue(ServerValue.TIMESTAMP);
                FirebaseDatabase.getInstance("https://chatting-a6254-default-rtdb.firebaseio.com/").getReference().child("users").child(CurrentUID).child("Seen").setValue("offline");
            }
        }
                    ...
                    if (dataSnapshot.exists()) {
                        for (DataSnapshot Snapshot : dataSnapshot.getChildren()) {
                            if (Snapshot.child("Message").getValue().toString().substring(0, 1).equals("S") && !Snapshot.child("Message State").getValue().toString().equals("3")) {
                                FirebaseDatabase.getInstance("https://chatting-a6254-default-rtdb.firebaseio.com/").getReference().child("chats").child(UserId).child(CurrentUserId).child(Snapshot.getKey().toString()).child("Message State").setValue("2");
                            }
                        }
                    }
                }
